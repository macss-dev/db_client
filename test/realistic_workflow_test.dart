...
SELECT s.nroDni, s.nroCuenta 
    ...
    test('Sequential queries with exception handling - 10 iterations', () async {
      print('\n${'=' * 80}');
      ...
      for (var i = 0; i < 10; i++) {
        ...
        try {
          ...
          await oracle.send(
            DbRequest.query('SELECT IDPRESTAMO FROM VPRESTAMO WHERE ROWNUM = 1'),
          );
          print('    ✓ Query OK');
    ...
    test('Database info queries - metadata operations', () async {
      ...
      for (var i = 0; i < 5; i++) {
        ...
        final oracleDbName = await oracle.send(
          DbRequest.query("SELECT SYS_CONTEXT('USERENV', 'DB_NAME') AS db_name FROM DUAL"),
        );
